#!/usr/bin/env python3
"""
Phase 11b test vectors — 4:1:1 YCbCr (Nf=3, H0=4,V0=1; chroma H=V=1; MCU=32x8).

Uses libjpeg-turbo `cjpeg -sample 4x1,1x1,1x1` for baseline 4:1:1 JPEGs.
...
OUT = ROOT / "verification" / "vectors" / "phase11b"
...
def cjpeg_encode(arr: np.ndarray, quality: int, restart: int,
                 out_path: Path) -> None:
    """Pipe a PPM (P6 RGB) through cjpeg -sample 4x1,1x1,1x1 to produce 4:1:1."""
    h, w, _ = arr.shape
    ppm = b"P6\n%d %d\n255\n" % (w, h) + arr.tobytes()
    args = [
        "cjpeg",
...
def main() -> int:
    ...
    # For 4:1:1 MCU is 32 wide x 8 tall, so aligned multiples of (32,8)
    cases = [
        # MCU-aligned (32x8)
        (32,  8,   "grad",  75, 0),
        (64,  16,  "check", 80, 0),
    ...
    for w, h, pat, q, r in cases:
        ...
        name = f"p11b_{pat}_{w}x{h}_q{q}_r{r}.jpg"
        p = OUT / name
        cjpeg_encode(img, quality=q, restart=r, out_path=p)
        verify_djpeg_ok(p)

    print(f"generated {len(cases)} Phase 11b 4:1:1 vectors under {OUT}")
    return 0

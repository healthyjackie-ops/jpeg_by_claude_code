#!/usr/bin/env python3
"""
Phase 10 test vectors — 4:2:2 YCbCr (Nf=3, H0=2,V0=1; chroma H=V=1; MCU=16x8).

Uses libjpeg-turbo `cjpeg -sample 2x1,1x1,1x1` for baseline 4:2:2 JPEGs.
Mix 16x8-aligned, non-aligned, DRI>0.
"""
...
def cjpeg_encode(arr: np.ndarray, quality: int, restart: int,
                 out_path: Path) -> None:
    """Pipe a PPM (P6 RGB) through cjpeg -sample 2x1,1x1,1x1 to produce 4:2:2."""
    h, w, _ = arr.shape
    ppm = b"P6\n%d %d\n255\n" % (w, h) + arr.tobytes()
    args = [
        "cjpeg",
...
def main() -> int:
    ...
    # For 4:2:2 MCU is 16 wide x 8 tall, so aligned multiples of (16,8)
    cases = [
        # MCU-aligned (16x8)
        (16,  8,   "grad",  75, 0),
        (32,  16,  "check", 80, 0),
    ...
    for w, h, pat, q, r in cases:
        ...
        name = f"p10_{pat}_{w}x{h}_q{q}_r{r}.jpg"
        p = OUT / name
        cjpeg_encode(img, quality=q, restart=r, out_path=p)
        verify_djpeg_ok(p)

    print(f"generated {len(cases)} Phase 10 4:2:2 vectors under {OUT}")
    return 0
